...
For the May 2026 launch cadence, use `GROWTH_RELEASE_PLAN.md` for weekly release goals and `marketing/launch-kit.md` for public announcement drafts.
...
`.changeset/config.json` keeps both packages in `ignore` and sets `privatePackages.version` / `privatePackages.tag` to `false`; `pnpm release:version` also runs a guard that fails if versioning touches either example package or creates an example-level `CHANGELOG.md`.
