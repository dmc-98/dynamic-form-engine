...
describe('createDfeRouter', () => {
  ...
  describeIfCanListen('GET /dfe/fields/:fieldId/options', () => {
    it('should require authentication', async () => {
      ...
      await request(app).get('/dfe/fields/f1/options').expect(401)
      expect(db.fetchFieldOptions).not.toHaveBeenCalled()
    })

    it('should only pass allowed filter keys', async () => {
      const app = createApp(db, {
        getUserId: () => 'user1',
        allowedOptionFilterKeys: ['departmentId'],
      })
    ...
    it('should reject filter values with injection characters', async () => {
      const app = createApp(db, {
        getUserId: () => 'user1',
        allowedOptionFilterKeys: ['category'],
      })
  ...
  describeIfCanListen('tenant isolation (requireTenantMatch)', () => {
    const submission = {
      id: 'sub-1',
      formId: 'form-1',
      userId: 'user1',
    ...
    it('denies cross-tenant access when requireTenantMatch is true', async () => {
      db.getSubmission.mockResolvedValue(submission)
      const app = createApp(db, {
        getUserId: () => 'user1',
      ...
      await request(app).get('/dfe/submissions/sub-1').expect(403)
    })

    it('denies access when request tenant is undefined and record is tenant-owned', async () => {
      db.getSubmission.mockResolvedValue(submission)
      const app = createApp(db, {
        getUserId: () => 'user1',
    ...
    it('allows matching tenant when requireTenantMatch is true', async () => {
      db.getSubmission.mockResolvedValue(submission)
      const app = createApp(db, {
        getUserId: () => 'user1',
        getTenantId: () => 'tenant-a',
      ...
      await request(app).get('/dfe/submissions/sub-1').expect(200)
    })
  })
})
