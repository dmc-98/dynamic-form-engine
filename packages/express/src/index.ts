...
export interface DfeRouterOptions {
   ...
   * Defaults to false.
   *
   * **DANGER:** This disables authentication AND ownership/tenant checks on
   * every submission endpoint. It must NEVER be enabled in production — it is a
   * development/testing convenience only. When enabled the router logs a loud
   * one-time warning at startup.
   */
  skipAuth?: boolean
  /**
   * Enforce tenant isolation on tenant-owned records (deny-by-default).
   * Defaults to `false` for backward compatibility, but production multi-tenant
   * deployments SHOULD set this to `true`. When `true`:
   *   - if a record has a non-null tenantId and the request's resolved tenant
   *     does not equal it → 403
   *   - if a record has a non-null tenantId but the request tenant is undefined
   *     → 403 (no longer fail-open)
   */
  requireTenantMatch?: boolean
  /**
   * Allowlist of context keys a client may supply on the step-submit endpoint.
   * The stored `submission.context` is always the base; only these keys from the
   * client-provided `context` are merged over it. Defaults to `[]`, meaning
   * client-provided context is ignored entirely. `userId` is always taken from
   * the authenticated principal and can never be overridden by the client.
   */
  allowedClientContextKeys?: string[]
  /**
   * Include per-form submission statistics (submissionCount, completionRate) in
   * the `GET /forms` list response. Defaults to `false` because computing these
   * stats issues an additional query per form (N+1). Enable only when the list
   * size is small or the adapter is optimized for it.
   */
  includeFormStats?: boolean
  /**
...
/**
 * Local narrow shape for the `dfe` block carried inside FormRuntimeContext.
 * FormRuntimeContext is defined in `@dmc--98/dfe-core` and must not be edited
 * from this package, so we cast through this local type once instead of using
 * scattered `(context as any)?.dfe` casts.
 */
interface DfeContextBlock {
  variantLabel?: string
  variantKey?: string
  [key: string]: unknown
}

function readVariantLabel(context: unknown): string | undefined {
  const dfe = (context as { dfe?: DfeContextBlock } | null | undefined)?.dfe
  return dfe?.variantLabel
}
...
export function createDfeRouter(options: DfeRouterOptions): Router {
  const {
    ...
    allowedClientContextKeys = [],
    includeFormStats = false,
    hipaa,
  } = options

  if (skipAuth) {
    // Loud one-time warning: skipAuth disables auth + ownership + tenant checks.
    console.warn(
      '[DFE] WARNING: createDfeRouter was created with skipAuth=true. ' +
      'Authentication, ownership, and tenant checks are DISABLED. ' +
  ...
  function verifyTenantAccess(req: Request, submissionTenantId: string | null | undefined, res: Response): boolean {
    ...
    const recordTenantId = submissionTenantId ?? null
    const tenantId = tenantIdForRequest(req)

    // Records without a tenant (public/global) are not tenant-scoped.
    if (recordTenantId === null) {
      return true
    }

    if (requireTenantMatch) {
      // Deny-by-default: an undefined request tenant can no longer fail open
      // against a tenant-owned record, and any mismatch is denied.
    ...
    // Backward-compatible mode: only enforce when the request carries a tenant
    // and it does not match the record (legacy fail-open when tenant undefined).
    if (tenantId !== undefined && tenantId !== recordTenantId) {
      res.status(403).json({ error: 'You do not have permission to access this tenant resource' })
      return false
    }
  ...
  router.get(`${prefix}/forms`, asyncHandler(async (req, res) => {
    ...
    // The adapter has no countSubmissions(); computing per-form stats requires
    // an extra listSubmissions()+getAnalyticsSummary() call per form (N+1). That
    // is opt-in via includeFormStats so the default list endpoint stays cheap.
    const items = includeFormStats
  ...
  router.post(`${prefix}/submissions/:id/steps/:stepId`, asyncHandler(async (req, res) => {
    ...
    // Untrusted client context: never let the client overwrite server-held
    // context. Start from the stored submission.context, then merge over only an
    // allowlisted subset of client-provided keys. userId always comes from the
    // authenticated principal, never from the client.
    const clientContext = (context && typeof context === 'object' ? context : {}) as Record<string, unknown>
    const mergedContext: Record<string, unknown> = { ...submission.context }
    for (const key of allowedClientContextKeys) {
      if (key === 'userId') continue
      if (Object.prototype.hasOwnProperty.call(clientContext, key)) {
        mergedContext[key] = clientContext[key]
      }
    }
    mergedContext.userId = userId

    const payloadContext = await maybeProtectContext(
      mergedContext as FormRuntimeContext,
      values ?? {},
      fieldPolicies,
      submission.context,
    )
    ...
    if (result.success) {
      ...
      await trackEvent({
        ...
        metadata: {
          ...
          variantLabel: readVariantLabel(result.context),
        },
        timestamp: Date.now(),
      })
      ...
      await Promise.all(fieldErrors.map(async ([fieldKey, error]) => {
        ...
        const event = sanitizeAnalyticsEventForCompliance({
          ...
          metadata: {
            ...
            error,
            variantLabel: readVariantLabel(submission.context),
          },
          timestamp: Date.now(),
  ...
  router.get(`${prefix}/fields/:fieldId/options`, asyncHandler(async (req, res) => {
    // Require authentication like the other endpoints (previously unauthenticated).
    const userId = authenticateRequest(req, res)
    if (!userId) return
    ...
    // TODO: scope dynamic options to the resolved tenant. The DatabaseAdapter
    // .fetchFieldOptions(fieldId, params) signature does not currently accept an
    // access/tenant context, so we cannot pass tenantIdForRequest(req) without a
    // breaking change. Extend the adapter interface to enable tenant scoping.
