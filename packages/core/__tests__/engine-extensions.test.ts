...
describe('Computed Fields', () => {
  ...
  it('should not break computed evaluation when a field key is a JS reserved word', () => {
    // Field keys like `default` or `arguments` are valid form keys but are JS
    // reserved words; they must be skipped during binding rather than crash the
    // Function-constructor based evaluator.
    const fields = [
      makeField({ key: 'default', type: 'NUMBER' }),
      makeField({ key: 'arguments', type: 'NUMBER' }),
      makeField({ key: 'rate', type: 'NUMBER' }),
      makeField({
        key: 'total',
        type: 'NUMBER',
        computed: {
          // References `rate`, a valid identifier; reserved keys are simply
          // not bound but their presence no longer throws.
          expression: 'rate * 2',
          dependsOn: ['rate', 'default', 'arguments'],
        },
      }),
    ]
    ...
    // Setting reserved-word keys must not throw and computed eval still works.
    engine.setFieldValue('default', 99)
    engine.setFieldValue('arguments', 7)
    engine.setFieldValue('rate', 5)

    expect(engine.getComputedValue('total')).toBe(10)
  })
