...
export function createFormEngine(
  ...
  return {
    ...
    getComputedValue(key: FieldKey): unknown {
      ...
      return evaluateExpression(config.expression, getCurrentValues(graph))
    },
...
/**
 * Set of JavaScript reserved words that cannot be used as binding identifiers.
 * Destructuring against these names is a syntax error, so they are skipped.
 */
const JS_RESERVED_WORDS = new Set([
  'arguments', 'eval', 'this', 'super', 'null', 'true', 'false',
...
/**
 * Evaluate a mathematical/logical expression with field values as context.
 *
 * SECURITY: This is NOT a sandbox. Expressions are evaluated via the `Function`
 * constructor and can run arbitrary JavaScript. They are intended to be TRUSTED,
 * author-supplied form configuration (e.g. computed-field formulas defined by the
 * form designer). They must NEVER be sourced from untrusted end users.
 *
 * Field values are exposed through a single `$scope` object and bound via
 * destructuring. Only keys that are valid, non-reserved JS identifiers are bound;
 * any other key (e.g. a field named `default` or `arguments`) is skipped so it
 * cannot break compilation of the evaluator.
 */
function evaluateExpression(expression: string, values: FormValues): unknown {
  try {
    const bindKeys = Object.keys(values).filter(
      k => VALID_IDENTIFIER.test(k) && !JS_RESERVED_WORDS.has(k),
    )
    const binding = bindKeys.length > 0
      ? `const { ${bindKeys.join(', ')} } = $scope;\n`
      : ''
    // eslint-disable-next-line no-new-func
    const evaluator = new Function('$scope', `${binding}return (${expression});`)
    return evaluator(values)
  } catch {
    // Library code must not log to the console; swallow and signal failure
    // to the caller via a null return value.
    return null
  }
}
    ...
    getNextBranch(): StepNodeState | null {
      ...
      for (const branch of currentStep.step.branches) {
        ...
        if (typeof branch.condition === 'string') {
          ...
          try {
            matches = !!evaluateExpression(branch.condition, values)
          } catch {
            matches = false
          }
        } else {
