...
/**
 * Fixed, documented HKDF salt. A constant salt is acceptable for HKDF because
 * the per-message security of AES-GCM comes from the random IV; the salt here
 * only domain-separates DFE key derivation from other uses of the same secret.
 */
const DFE_HKDF_SALT = 'dfe:protected-field:v1'
...
export function createAesGcmFieldProtector(options: {
  ...
  // Cache derived AES-GCM keys per keyId so different keyIds derive different
  // keys (enabling rotation) and decrypt can honor the keyId stored on a value.
  const keyCache = new Map<string, Promise<unknown>>()

  // NOTE: This previously used SHA-256(secret) directly as the AES key with no
  // salt and ignored keyId on decrypt. We now derive the key with HKDF-SHA256
  // (salt = DFE_HKDF_SALT, info incorporates the keyId) via Web Crypto's
  // deriveBits — the portable HKDF available across Node, browsers, and Workers.
  // (The audit suggested crypto.hkdfSync, but that is Node-only and would break
  // this module's intentional Web Crypto portability.)
  // BACKWARD COMPATIBILITY: this changes the derived key, so values encrypted by
  // the old SHA-256(secret) scheme are NOT decryptable after upgrade. Operators
  // must re-encrypt existing vaults (rotate) when adopting this version.
  function deriveKey(keyId: string | undefined): Promise<unknown> {
    ...
    if (!cached) {
      cached = (async () => {
        ...
        const info = encoder.encode(`dfe:aes-gcm:${keyId ?? 'default'}`)
        const derivedBits = await crypto.subtle.deriveBits(
          {
            name: 'HKDF',
            hash: 'SHA-256',
            salt: encoder.encode(DFE_HKDF_SALT),
            info,
          },
          baseKey,
          256,
  ...
  return {
    async encrypt(value) {
      const crypto = await resolveWebCrypto()
      const key = await deriveKey(options.keyId)
    ...
    async decrypt(value) {
      const crypto = await resolveWebCrypto()
      // Honor the keyId stored on the value: re-derive with that id so values
      // encrypted under a previous keyId still decrypt after a rotation.
      const key = await deriveKey(value.keyId)
