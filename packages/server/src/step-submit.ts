...
export function resolveEndpoint(template: string, context: FormRuntimeContext): string {
  return template.replace(/{(\w+)}/g, (_, key) => {
    ...
    // Encode interpolated values so context-supplied data can't inject extra
    // path segments or query parameters into the resolved endpoint.
...
export async function executeStepSubmit(
  ...
  for (const contract of contracts) {
    try {
      ...
      // Info disclosure: keep the real downstream error in server logs only and
      // return a generic message to the client so internal endpoints, hostnames,
      // and stack details are not leaked.
      console.warn(
        `[DFE] API contract for ${contract.resourceName} failed:`,
      ...
      return {
        ...
        errors: {
          _api: 'A downstream service call failed',
        },
      }
    }
  }
  ...
  try {
    ...
    // Persistence failed: do NOT report success, otherwise the client and
    // analytics would believe the step advanced while the server dropped it.
    console.warn(
    ...
    return {
      success: false,
      context: updatedContext,
      errors: { _persist: 'Failed to persist step' },
    }
  }
