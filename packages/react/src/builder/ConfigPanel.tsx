...
function SelectConfigPanel({ config, onChange }: SelectConfigPanelProps) {
  ...
  // The static `SelectOption` data model has no id, and option value/label may
  // be empty or duplicated while editing — so neither is a safe React key.
  // Maintain a parallel array of stable row ids that tracks each option's
  // identity across re-renders, keeping it in sync with the options length so
  // that deleting a row removes the correct DOM node (and preserves focus on the
  // remaining inputs) instead of shifting state up by index.
  const optionsLength = (config.options ?? []).length
  const rowIdSeq = useRef(0)
  const rowIdsRef = useRef<string[]>([])
  if (rowIdsRef.current.length < optionsLength) {
    while (rowIdsRef.current.length < optionsLength) {
      rowIdsRef.current.push(`opt-${rowIdSeq.current++}`)
    }
  } else if (rowIdsRef.current.length > optionsLength) {
    rowIdsRef.current = rowIdsRef.current.slice(0, optionsLength)
  }

  const removeOptionAt = (index: number) => {
    rowIdsRef.current = rowIdsRef.current.filter((_, j) => j !== index)
    const updated = (config.options ?? []).filter((_, j) => j !== index)
      ...
      {config.mode === 'static' && (
        <div data-dfe-builder-section>
          ...
          {(config.options ?? []).map((opt, i) => (
            <div key={rowIdsRef.current[i]} style={{ display: 'flex', gap: '0.5rem' }}>
              <input
                type="text"
                value={opt.label}
                onChange={e => {
