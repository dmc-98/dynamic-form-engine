  ...
  return (
    <div data-dfe-builder-mapping>
      {entries.map(([key, value]) => (
        // The mapping is a Record keyed by the "from" field, so the from-key is
        // the row's natural stable identity. Using it (instead of the array
        // index) keeps focus on the correct input when a row is removed.
...
export function StepConfigPanel({ config, onChange, className }: StepConfigPanelProps) {
  ...
  // `StepApiContract` has no id and its editable fields can be empty/duplicated
  // while editing, so neither is a safe React key. Track a parallel array of
  // stable row ids kept in sync with the contracts length so removing a contract
  // unmounts the correct editor (preserving input focus/state) rather than
  // shifting every row's state up by one index.
  const contractIdSeq = useRef(0)
  const contractIdsRef = useRef<string[]>([])
  if (contractIdsRef.current.length < contracts.length) {
    while (contractIdsRef.current.length < contracts.length) {
      contractIdsRef.current.push(`contract-${contractIdSeq.current++}`)
    }
  } else if (contractIdsRef.current.length > contracts.length) {
    contractIdsRef.current = contractIdsRef.current.slice(0, contracts.length)
  }
  ...
  const removeContract = useCallback((index: number) => {
    contractIdsRef.current = contractIdsRef.current.filter((_, i) => i !== index)
