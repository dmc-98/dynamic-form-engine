...
import { getStarterFormScaffold, listStarterForms } from '../starter-form'
...
export const initCommand = new Command('init')
  ...
  .option('--template <id>', 'Scaffold a starter form (onboarding | application | workflow)')
...
`
    ...
    // Optionally scaffold a starter form from a built-in template.
    if (opts.template) {
      const scaffold = getStarterFormScaffold(opts.template)
      if (!scaffold) {
        const aliases = listStarterForms().map(s => s.alias).join(', ')
        console.log(chalk.yellow('  !'), `Unknown template "${opts.template}". Available: ${aliases}`)
      } else {
        const starterPath = join(dir, 'src/forms', scaffold.filename)
        if (!existsSync(starterPath)) {
          writeFileSync(starterPath, scaffold.code)
          console.log(chalk.green('  ✓'), `Created src/forms/${scaffold.filename}`)
        }
      }
    }
