...
describe('DrizzleDatabaseAdapter', () => {
  ...
  describe('fetchFieldOptions', () => {
    it('should push search to SQL and apply meta filters + keyset cursor', async () => {
      const o1 = { id: 'o1', fieldId: 'f1', label: 'Platform Engineering', value: 'platform', meta: { department: 'eng' }, order: 1 }
      const o2 = { id: 'o2', fieldId: 'f1', label: 'Infrastructure', value: 'infrastructure', meta: { department: 'eng' }, order: 2 }

      // Search is pushed into SQL (`ilike`), so the mock returns what the DB
      // would return for `search: 'ing'` — only the matching row.
      ;(db.select as any).mockReturnValueOnce(createChainableMock([o1]))
      ...
      // First page (size 1): DB returns pageSize+1 rows for has-more detection.
      ;(db.select as any).mockReturnValueOnce(createChainableMock([o1, o2]))
      ...
      // Cursor path does TWO selects: (1) look up the cursor row, (2) the page.
      ;(db.select as any)
        .mockReturnValueOnce(createChainableMock([o1])) // cursor lookup
        .mockReturnValueOnce(createChainableMock([o2])) // keyset page
    ...
    it('should return an empty page when the cursor row is not found', async () => {
      ;(db.select as any).mockReturnValueOnce(createChainableMock([])) // cursor lookup misses
