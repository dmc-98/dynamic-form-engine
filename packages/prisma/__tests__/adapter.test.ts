...
describe('PrismaDatabaseAdapter', () => {
  ...
  describe('fetchFieldOptions', () => {
    ...
    it('should push search to SQL and apply meta filters + keyset cursor', async () => {
      // Search is now pushed into the query (Prisma `contains`), so the mock
      // returns what the DB would return for `search: 'ing'` — only the row
      // whose label contains "ing". The adapter then applies the meta filter.
      prisma.dfeFieldOption.findMany.mockResolvedValueOnce([
      ...
      // The query must carry the search predicate into the DB layer.
      const firstArgs = prisma.dfeFieldOption.findMany.mock.calls[0][0]
      expect(JSON.stringify(firstArgs)).toContain('ing')
      ...
      // Next page: the DB (keyset on the cursor) returns the following row.
      prisma.dfeFieldOption.findMany.mockResolvedValueOnce([
        { id: 'o2', label: 'Infrastructure', value: 'infrastructure', meta: { department: 'eng' } },
      ])
      ...
      // The cursor must be passed to the DB query (keyset pagination).
      const cursorArgs = prisma.dfeFieldOption.findMany.mock.calls.at(-1)![0]
      expect(JSON.stringify(cursorArgs)).toContain('o1')
    })
