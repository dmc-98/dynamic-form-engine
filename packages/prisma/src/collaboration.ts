...
export class PrismaCollaborationStore implements CollaborationStore {
  ...
  async listEvents(
    ...
    // Run the access check and the read in a single transaction so the session
    // we authorized against is the same snapshot we list events from.
    const events = await this.prisma.$transaction(async (tx) => {
      await this.requireSession(tx, sessionId, access)
      return tx.dfeCollaborationEvent.findMany({
        where: {
          sessionId,
          sequence: {
  ...
  async listPresence(sessionId: string, access?: CollaborationAccessContext): Promise<SyncPresence[]> {
    // Access check + read together, so listing is consistent with authorization.
    const rows = await this.prisma.$transaction(async (tx) => {
      await this.requireSession(tx, sessionId, access)
      return tx.dfeCollaborationPresence.findMany({
        where: { sessionId },
        orderBy: { updatedAt: 'desc' },
      })
    })
  ...
  async prunePresence(sessionId: string, olderThanMs: number, access?: CollaborationAccessContext): Promise<number> {
    // Mutating operation: the access check and the delete must be atomic so we
    // never prune presence for a session the caller wasn't authorized against.
    const result = await this.prisma.$transaction(async (tx) => {
      await this.requireSession(tx, sessionId, access)
      const threshold = new Date(Date.now() - olderThanMs)
      return tx.dfeCollaborationPresence.deleteMany({
        where: {
          sessionId,
          updatedAt: {
            lt: threshold,
