...
function assertChangesetIgnoresPrivateExamples() {
  ...
  assert.equal(
    ...
    false,
    'Changesets privatePackages.version must be false so private example apps are never versioned',
  )
  assert.equal(
    config.privatePackages?.tag,
    false,
    'Changesets privatePackages.tag must be false so private example apps are never tagged',
  )
...
function assertPrivateExampleAppsStayPrivate() {
  for (const packageName of ignoredPrivateWorkspaceApps) {
    ...
    assert.equal(
      ...
      false,
      `${packageName} must not have a package-level CHANGELOG.md`,
    )
  }
}
